...
function trivy_scan(){
  ...
  do
    ...
    trivy --exit-code 1 --quiet --ignore-unfixed --skip-dirs "opt/td-agent/embedded/lib/ruby/gems" --severity HIGH,CRITICAL "${image_name}"
    EXITCODE=$?
    if [ $EXITCODE -eq 1 ]; then
      # Write vulnerability details to log file
      trivy --quiet --ignore-unfixed --skip-dirs "opt/td-agent/embedded/lib/ruby/gems" --severity HIGH,CRITICAL "${image_name}" >> trivy_report.log
      trivy --quiet --ignore-unfixed --skip-dirs "opt/td-agent/embedded/lib/ruby/gems" --severity HIGH,CRITICAL -f json "${image_name}" >> trivy_report.json
    fi
  done
